...
#Two ensembles are created to represent the matrices A and B
    ...
    A = nengo.Ensemble(n_neurons=400, dimensions=4)
...
#The functions A_fun and B_fun model the terms f'=TAx and g'=TBu. 
    ...
    nengo.Connection(B,A,synapse = t_synapse,function=B_fun)
    nengo.Connection(A,A,synapse = t_synapse,function=A_fun)
...
#transform corresponding to the control gains to generate the control signals
    ...
    nengo.Connection(error,B,function=control)        
    ...
    nengo.Connection(d_e_x,B,function=de_x,transform=kd_x)
    nengo.Connection(d_e_y,B,function=de_y,transform=kd_x)
    
    ens_probe = nengo.Probe(A,synapse=0.1)
